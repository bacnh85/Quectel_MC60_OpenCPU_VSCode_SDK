...
static u8  m_SrvADDR[SRVADDR_BUFFER_LEN] = "124.74.41.170";
static u8  m_ipaddress[5];  //only save the number of server ip, remove the comma
static u32 m_SrvPort = 8010;
        ...
        switch (m_udp_state)
        {
            ...
            case STATE_GPRS_GET_LOCALIP:
            {
                ...
                if (ret == GPRS_PDP_SUCCESS)
                {
                    ...
                    m_udp_state = STATE_CHACK_SRVADDR;
                }else
                {
            ...
            case STATE_SOC_SEND:
            {
                ...
                do
                {
                    ret = Ql_SOC_SendTo(m_socketid, m_pCurrentPos, m_remain_len, (u32)m_ipaddress, m_SrvPort);
